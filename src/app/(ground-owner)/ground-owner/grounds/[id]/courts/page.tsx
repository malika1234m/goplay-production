...
import {
  ...
  CheckCircle, XCircle, GripVertical, Eye, EyeOff, Lock,
} from "lucide-react";
...
function CourtModal({ court, onClose, onSaved, facilityId }: CourtModalProps) {
  ...
  const handleSave = async () => {
    ...
    if (!res.ok) {
      setError(data.code === "PLAN_LIMIT" ? "PLAN_LIMIT:" + data.error : (data.error ?? "Failed to save court."));
    } else {
      onSaved(data.court);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-5" onClick={(e) => e.stopPropagation()}>
        ...
        {error && error.startsWith("PLAN_LIMIT:") ? (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2.5">
            ...
            <div>
              <p className="text-xs font-semibold text-amber-800">{error.slice(11)}</p>
