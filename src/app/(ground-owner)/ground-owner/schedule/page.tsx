...
function DayView({
  ...
  const columns: Col[] = hasCourts
    ? [
        ...
        // Legacy/general bookings with no court assigned
            ...
            return (
              <button
                ...
                className={`flex-1 px-2 py-2.5 text-center border-l border-slate-100
                            hover:bg-green-50 transition-colors group ${isToday ? "bg-green-50" : ""}`}
              >
                ...
                <p className={`text-lg font-bold leading-tight
                               ...
                               ${isToday ? "text-green-700" : "text-slate-800"}`}>
                  {day.getDate()}
                </p>
                {sched?.isOpen
        ...
        {cells.map((day, idx) => {
          ...
          return (
            ...
            >
              ...
              {hasEvents && (
                <p className="text-[8px] text-slate-300 font-medium mt-1 group-hover:text-green-400 transition-colors">
                  View day →
                </p>
              )}
            </button>
...
export default function SchedulePage() {
  ...
  // Reset court filter when facility changes
  useEffect(() => { setCourtId(""); }, [facilityId]);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const params = new URLSearchParams({ facilityId, from, to });
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Bookings, maintenance blocks, and availability in one view
          </p>
        </div>
          ...
          {facilities.length > 1 && (
            <select
              ...
              className="text-sm border border-slate-200 rounded-lg px-3 py-2 text-slate-700
                         outline-none focus:ring-2 focus:ring-green-500 bg-white"
            >
              {facilities.map((f) => (
