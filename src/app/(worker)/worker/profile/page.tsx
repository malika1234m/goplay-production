...
function initials(name: string) { return name?.[0]?.toUpperCase() ?? "W"; }
function avatarColor(name: string) {
  const palette = ["from-blue-500 to-cyan-600","from-indigo-500 to-blue-600","from-violet-500 to-indigo-600","from-sky-500 to-blue-600","from-cyan-500 to-teal-600"];
...
export default function WorkerProfilePage() {
  ...
  const [workerSince, setWorkerSince] = useState<string | null>(null);
  ...
  useEffect(() => {
    fetch("/api/worker/profile").then((r) => r.json()).then((d) => {
      if (d.user) {
        setUser(d.user);
        setPForm({ name: d.user.name ?? "", phone: d.user.phone ?? "" });
      }
      if (d.facility)    setFacility(d.facility);
      if (d.workerSince) setWorkerSince(d.workerSince);
  ...
  return (
    <div className="max-w-3xl flex flex-col gap-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
        ...
        {/* Banner */}
        <div className="bg-gradient-to-br from-blue-600 to-blue-500 px-5 sm:px-8 py-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              {facility && (
                <p className="text-blue-100 text-sm font-medium mt-0.5 flex items-center gap-1.5">
                  <Building2 className="w-3.5 h-3.5 shrink-0" />{facility.name}
                </p>
              )}
          ...
          <div className="grid grid-cols-3 gap-3 mt-7 pt-6 border-t border-white/20">
            {[
              ...
              <div key={label} className="text-center">
                ...
                <p className={`text-white font-bold ${small ? "text-sm leading-snug" : "text-xl"}`}>{value}</p>
                <p className="text-blue-200 text-xs mt-0.5 leading-tight">{label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Facility info strip */}
        {facility && (
          <div className="flex items-center gap-3 px-6 py-3 border-b border-slate-100 bg-blue-50/50">
