...
export default function WorkerMaintenancePage() {
  ...
  const [form, setForm] = useState({
    date:"", blockType:"full" as "full"|"hours", startTime:"08:00", endTime:"10:00", reason:"",
  });
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/worker/blocked-dates").then((r) => r.json()),
    ]).then(([facData, blData]) => {
      setFacilityName(facData.facility?.name ?? blData.facilityName ?? "");
  ...
  const addEntry = async () => {
    ...
    try {
      const body: Record<string,string> = { date:form.date, reason:form.reason };
      if (form.blockType==="hours") { body.startTime=form.startTime; body.endTime=form.endTime; }
      const res  = await fetch("/api/worker/blocked-dates",{
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-wrap gap-3">
            <input type="date" value={form.date} min={new Date().toISOString().split("T")[0]}
              ...
              className="text-sm border border-slate-200 rounded-lg px-3 py-2.5 text-slate-700 outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
          <div className="flex gap-2">
            {(["full","hours"] as const).map((bt) => (
              <button key={bt} type="button" onClick={() => setForm((f)=>({...f,blockType:bt}))}
                className={`px-4 py-2 text-sm font-medium rounded-lg border-2 transition-all ${
                  form.blockType===bt
                    ? bt==="full" ? "border-orange-500 bg-orange-50 text-orange-700"
            ...
            {upcoming.map((b) => (
              <div key={b.id} className="px-6 py-4 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${b.startTime?"bg-red-50 text-red-500":"bg-orange-50 text-orange-500"}`}>
                ...
                <button onClick={() => removeEntry(b.id)} disabled={removing===b.id}
